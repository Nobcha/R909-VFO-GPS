...
//  LCD pin
//  RE pin D3, D4, A0
//  przycisk is assigned as RE-SW  L#32, L#78, L#105, L#189
//  EN & LCD B.L  L#45 L#88  < LDO_enable
//  Changed GPS port for SoftwareSerial  L#28 L#99 L#582
//  Replace CLK0:VFO OUT with CLK1:25MHz  L#96, 102, 103, 104, 105, 561
//  softwareserial confricts  RE interrupt goes to reset
//  D5:2.5MHz  softwareserial 

//　2024.10.29　このスケッチの出所GITHUB https://github.com/csqwdy/gps-calibration-5351
//  動作不安定　RE回すとリセットする。softwareserialは問題なしと思う。
//  割り込みフラグ設定に問題ありと推定。

//2021-12-23
//Add control for GPS power supply, so that you can easily use USB to burn firmware
//Modify the frequency range to 1KHz-200MHz

//2022-03-06
//Modify the encoder to work in interrupt mode, the encoder becomes more useful
...
// Set up MCU pins
#define LDO_Enable               7    // LCD back light & GPS Enable <6
#define ppsPin                   2
#define przycisk                A0    // <A2
#define sw_up					           4    // <A1
#define sw_down					         3    // <A0
#define RS                       8    // <7
#define E                        9    // <8
...
void setup()
{
  ...
  pinMode(LDO_Enable, OUTPUT);          // GPS Power control pin enable
  digitalWrite(LDO_Enable, HIGH);       // GPS The power control pin outputs a high level
...
//
  PCICR =  0b00000110;                  // 1. PCIE2: Pin Change Interrupt Enable D0-D7
  PCMSK2 = 0b00001100;                  // Enable Pin Change Interrupt for D3, D4
  PCMSK1 = 0b00000001;                  // Enable Pin Change Interrupt for A0
  ...
  pinMode(ppsPin, INPUT);                        // Inititalize GPS 1pps input as D2
  digitalWrite(ppsPin, HIGH);


  lcd.begin(16, 2);
  lcd.display();
  if (digitalRead(przycisk) == 0) {     // If RE-SW on when starting up
...
void loop()
{
  ...
  //The encoder is changed to interrupt mode
  if (left) {
    left = false;
    change_down();     
  }
